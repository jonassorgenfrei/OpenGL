...
void main() 
{
	/* Note: convert albedo to linear space, since albedo in general authored in SRGB */
	vec3 albedo = pow(texture(albedoMap, TexCoords).rgb, vec3(2.2)); // convert to linear space
	/* metallic & roughness: almost always in linear space */
	...
	// ambient can be replaced with environment lighting (IBL)
