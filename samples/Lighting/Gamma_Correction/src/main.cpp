/* 
 *	Gamma Correction
 *		applying the inverse of the monitor's gamma to the final color before displaying to the monitor
 *		=> multiply each of the linear output colors by the inverse gamma curve of the monitor (making them brighter)
 *			the monitor itself does the inverse and the monitor finally displays the colors as we linearly set them
 *		transforms colors from linear space to non-linear space
 *		IMPORTANT: only at the last/final step
 */
...
int main()
{
	...
	// OpenGL automatically performs gamma correction (value = 2.2) after each frag. Shader to 
	// all subsequent framebuffers, incl. default Framebuffer
	// SRGB color Space of Gamma 2.2
	//glEnable(GL_FRAMEBUFFER_SRGB);

	// build and compile our shader program
	// ------------------------------------
	Shader shader(FileSystem::getSamplePath("shader/blinnPhongShader.vert").c_str(), FileSystem::getSamplePath("shader/blinnPhongShader.frag").c_str());
	...
	unsigned int floorTexture = loadTexture(FileSystem::getPath("content/images/wood.png").c_str(), false);
	unsigned int floorTextureGammaCorrected = loadTexture(FileSystem::getPath("content/images/wood.png").c_str(), true);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		std::cout << (gammaEnabled ? "Gamma enabled" : "Gamma disabled") << " " << (blinn ? "blinn enabled" : "blinn disabled")  << std::endl;
...
{	
	...
	// Activate/Deactivate Gamma Correction
