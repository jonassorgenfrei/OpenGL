...
int main()
{
	...
	// ------------------------------------
	Shader shader(FileSystem::getPath("shader/lighting.vs").c_str(), FileSystem::getPath("shader/lighting.fs").c_str());
	Shader hdrShader(FileSystem::getPath("shader/hdrShader.vs").c_str(), FileSystem::getPath("shader/hdrShader.fs").c_str());
	// load textures
	// -------------
	...
	// 16 bits per color componentn (2 times more memory for storing color values [then prev. 8 bits])
	// as 32 bits isn't really neessary (when high precision isn't neee)
	...
	lightColors.push_back(glm::vec3(0.1f, 0.0f, 200.0f));
