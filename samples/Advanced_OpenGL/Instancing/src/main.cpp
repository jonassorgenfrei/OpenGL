...
int main()
{
	...
	// ------------------------------------
	Shader shader(FileSystem::getSamplePath("shader/instance_shader.vert").c_str(), FileSystem::getSamplePath("shader/instance_shader.frag").c_str());
	...
	/* Allows to pass a lot more data */
	...
	for (unsigned int i = 0; i < 100; i++) {
		stringstream ss;	//dynamically create a location string for querying the uniform location
		string index;
		ss << i;
		index = ss.str();
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//			1 -> update content of the vertex attribute when we start to render a new instance (effectively:  vert. attrib at attrib loc 2 is an instanced array)
		//			2 -> update content every 2 instances and so on
