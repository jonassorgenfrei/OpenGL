...
int main()
{
	...
	// ------------------------------------
	Shader shader(".\\shader\\vertexShader.vs", ".\\shader\\fragmentShader.fs", nullptr, ".\\shader\\tessellationShader.tesc", ".\\shader\\tessellationShader.tese");
	
	// load textures
	// -------------
	/* Bricks
	unsigned int diffuseMap = loadTexture(".\\images\\bricks2.jpg",false);
	unsigned int depthMap = loadTexture(".\\images\\bricks2_disp.jpg", false); // DepthMap => inverse of the heightmap (easier to fake depth instead of height)
	unsigned int normalMap = loadTexture(".\\images\\bricks2_normal.jpg", false);
	*/

	/* Bricks */
	unsigned int diffuseMap = loadTexture(".\\images\\wood.png",false);
	unsigned int depthMap = loadTexture(".\\images\\toy_box_disp.png", false); // DepthMap => inverse of the heightmap (easier to fake depth instead of height)
	unsigned int normalMap = loadTexture(".\\images\\toy_box_normal.png", false);
	...
	// Driver Definies max patch Vertices
	glGetIntegerv(GL_MAX_PATCH_VERTICES, &maxPatchVertices); // fetch Value
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		shader.setFloat("gDispFactor", heightScale);
